...
from pages.getEmail import getEmail
...
'''TRK-030 Проверка восстановления пароля по почте - негативные сценарии. В начале теста в поле  Символы требуется одноразовый ручной ввод CAPTCHA.'''
...
def test_forgot_password_page(browser):
    ...
    result_id, status_id = getEmail().get_id_letter(mail_name, mail_domain)
    # Получаем id письма с кодом из почтового ящика:
    id_letter = result_id[0].get('id')
    # Проверяем полученные данные с  ожиданиями
    ...
    result_code, status_code = getEmail().get_reg_code(mail_name, mail_domain, str(id_letter))
    ...
    # Проверка полученных данных с ожиданиями
